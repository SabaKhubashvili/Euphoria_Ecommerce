...
export const MainTable = ({ type, topContent, bodyContent, notFoundMessage }: Props) => {
  ...
  return (
    ...
    <div className={`flex items-center w-full py-[8px] px-[20px] border-b-[1px] border-[#DBDADE] border-solid ${type === "primary" ? "" : type === "secondary" ? "bg-[#F8F9FA]" : ""}`}>
    ...
    <div className={`flex flex-col h-full overflow-y-auto w-full`}>
      {bodyContent.length > 0 ? (
        bodyContent.map((cont: any, index: number) => (
          <div key={index} className={`flex items-center py-[8px] px-[20px]     ${type === 'primary' && 'border-b-[1px] border-solid border-[#E9E7FD]'}`}>
            ...
            <div style={{ flexBasis: `${100 / topContent.length}%` }} className="flex justify-end">
              <Dropdown_Down />
            </div>
          </div>
        ))
