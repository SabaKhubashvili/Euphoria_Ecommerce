...
class MainTableRows {
    static HeaderCell = ({ text, tableData }: { text: string; tableData: any[] }) => (
        <h1 className={`text-secondaryGray uppercase font-medium md:text-[16px] text-[13px]`} style={{ flexBasis: 100 / (Object.keys(tableData[0]).length + 1) + '%' }}>
            {text}
        </h1>
    );
    ...
    static StatusRowCell = ({
        ...
        <h1
            className={`font-medium xl:text-[16px] text-[13px] cursor-pointer flex items-center justify-between rounded-[5px]  !py-[5px] !px-[10px] ${getCellClasses(
                text,
                type
            )}`}
